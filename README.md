...
它以 Obsidian 知识库为唯一事实源，通过 《Stellaris》 风格的三层视图（世界卡片 → 星域地图 → 恒星系详情）呈现地理层级，并为 Markdown 附加可编辑的坐标元数据，实现画布与笔记的双向同步。
...
- **Azgaar FMG 底图导入**：支持导入 Azgaar Fantasy Map Generator 的 `.map` 文件，在行星地图中显示政治实体边界、自然区划轮廓、海岸线/山脊线、文化/宗教区域作为绘画参考（可选功能，不导入 .map 也能使用全部核心功能）
...
司天支持导入 [Azgaar Fantasy Map Generator](https://azgaar.github.io/Fantasy-Map-Generator/) 导出的 `.map` 文件，在行星地图中显示以下参考图层：

| 图层 | 说明 | 显示方式 |
|------|------|----------|
| 政治实体边界 | FMG 中的国家/省份边界 | 半透明虚线（可切换为可编辑模式） |
| 自然区划轮廓 | 生物群系（森林/沙漠/苔原…）之间的边界 | 白色细线 |
| 海岸线/山脊 | 海平面等值线 + 高度梯度大的边 | 蓝色实线（海岸）+ 橙色虚线（山脊） |
| 文化/宗教区域 | FMG 中的文化/宗教分布 | 半透明色块叠加 |

**导入方式**：在剧本地图（ScenarioMap）工具栏点击"导入 .map 底图"，系统会自动将数据同步到对应的行星地图中。

> 注意：Azgaar 底图导入是可选增强功能。不导入 .map 文件时，司天的所有核心功能（地形绘制、聚落放置、高度笔刷等）均可正常使用。
...
5. **外部数据可选**：Azgaar .map 等第三方数据仅作为绘画参考，不导入也能使用全部核心功能
...
- **[Azgaar Fantasy Map Generator](https://azgaar.github.io/Fantasy-Map-Generator/)** — 提供了 `.map` 文件格式解析的基础，司天的政治实体边界、文化/宗教区域、自然区划轮廓等功能依赖该工具导出的数据
- **[Vue.js](https://vuejs.org/)** — 前端框架
- **[Electron](https://www.electronjs.org/)** — 桌面应用壳
