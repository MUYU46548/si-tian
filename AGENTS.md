...
- **测试基线**: 两层。① `scripts/tests/unit/*.js` Node 单元测试（**主进程侧**：`.sitian` 原子写/备份轮转/路径守卫 + `index.js` 模块接线不变式 + `config.js` 读回校验——CDP 用例里 `window.sitianAPI` 是 mock，测不到真实落盘与主进程接线）；② `scripts/tests/cases/` 49 个 CDP 用例（Edge 驱动），全绿 + Node 层通过 = 迁移/重构完整。`run_tests.py` 把前者作为前置步骤，失败计 1 个失败用例；**每个用例前还会打开 harness 基线项目**（决策 1 终态下无项目=只读，用例必须跑在有项目的状态，见写闸门条目）
