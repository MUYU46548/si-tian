...
- 回归测试: `python scripts/tests/run_tests.py`（22 用例；须用系统 Python，Hermes 自带 venv 缺 `websocket-client`）
...
- **测试基线**: `scripts/tests/cases/` 22 个用例（Edge CDP 驱动），22/22 全绿 = 迁移/重构完整
...
| 锚点 | 期望值 | 核对方式 |
|---|---|---|
| 测试用例数 | 22 | `ls scripts/tests/cases/test_*.py \| wc -l` |
