...
- Canvas 绘制函数拆分时，辅助函数和样式常量必须随函数一起迁移，否则运行时报 ReferenceError 导致渲染管线断裂
- 五层视图：世界 → 星域 → 恒星系 → 行星地图 → 区域地图（AreaMap）
- 行星地图只显示聚落（城市/城镇/村庄）+ 地点，地形作为环境背景保留编辑能力但不参与行政层级
- 点击聚落/地点 → 进入区域地图（AreaMap）编辑子节点；第三层下钻（建筑内部）已确认需要
- 区域层为行星地图可切换图层
- 暂存地点（无 Obsidian 词条）用虚线边框标记
