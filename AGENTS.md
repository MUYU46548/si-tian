...
- 回归测试: `python scripts/tests/run_tests.py`（48 用例；须用系统 Python，Hermes 自带 venv 缺 `websocket-client`）
...
- **测试基线**: 两层。① `scripts/tests/unit/*.js` Node 单元测试（**主进程侧**：`.sitian` 原子写/备份轮转/路径守卫 + `index.js` 模块接线不变式 + `config.js` 读回校验——CDP 用例里 `window.sitianAPI` 是 mock，测不到真实落盘与主进程接线）；② `scripts/tests/cases/` 50 个 CDP 用例（Edge 驱动），全绿 + Node 层通过 = 迁移/重构完整。`run_tests.py` 把前者作为前置步骤，失败计 1 个失败用例；**每个用例前还会打开 harness 基线项目**（决策 1 终态下无项目=只读，用例必须跑在有项目的状态，见写闸门条目）
...
- **单一写闸门（Phase 2，2026-09-18；终态 2026-09-20 落地）**: `store/writeGate.js` = 世界观数据**落盘写**的唯一判定入口（`guardWrite()` + `isReadOnly`）。三模式 `project` / `legacy` / `readonly`，对应决策「有项目文件用项目、无项目回退 Obsidian，但**无项目必须只读**」——该终态已翻开：`READONLY_WITHOUT_PROJECT = true`。**11 条落盘入口全部已守 + 只读态 UI 灰禁已落**，test_47 读源码校验（含只读灰禁子测试）。两条伴随纪律：① **项目文件的生命周期操作（新建/打开/保存/备份）不受本闸门管辖** —— 否则「无项目 → 只读 → 禁新建」死锁，第一个项目永远打不开（ProjectPanel 的「新建」必须保持可用）；② **回归基线必须跑在「已打开项目」态**：`run_tests.py` 每个用例前用当前知识库内容播种并打开一个 mock 基线项目（`lib/helpers.py#open_harness_project`，含 `window.__probe` 探针），既有落盘断言一律改读**项目文件**而非 legacy IPC
...
| 锚点 | 期望值 | 核对方式 |
|---|---|---|
| 测试用例数 | 52 | `ls scripts/tests/cases/test_*.py \| wc -l` |
