...
| 216 | `scripts/tests/lib/helpers.py` | 测试公共 helper（世界/行星导航锚定） |
| 258 | `scripts/tests/run_tests.py` | 测试主控（Edge CDP + mock 注入，用后还原） |
...
| 2320 | `src/renderer/src/components/AreaMap.vue` | 区域地图（行星下钻）：区域多边形/道路/标记/文本/建筑内部入口 |
...
| 163 | `src/renderer/src/components/Icon.vue` | （待补） |
...
| 1719 | `src/renderer/src/components/NodeDetailPanel.vue` | 节点详情（正文/层级迁移/定位；含 space_marker、fleet_card 伪节点适配） |
...
| 2976 | `src/renderer/src/components/PlanetMap.vue` | 行星地图（最大组件）：地形/聚落/批量操作，装配 22 个 composables；**读片段勿整读** |
...
| 2483 | `src/renderer/src/components/ScenarioMap.vue` | 剧本地图全屏工作台：底图省份绘制/拆分合并/顶点编辑（贝塞尔切线手柄 + 海岸线吸附 + 网格吸附）、剧本时间轴与势力染色、FMG .map 数据图层（陆海底色/地形高度/温度/降水栅格 + 河流/道路 + 文化/宗教着色与图例）、城镇图层与右键属性面板、PNG 导出 |
| 612 | `src/renderer/src/components/SearchBar.vue` | 全局搜索（store/geodataModules/search.js） |
| 770 | `src/renderer/src/components/SettingsPanel.vue` | 设置面板（选库/关闭行为/窗口模式） |
...
| 1652 | `src/renderer/src/composables/planetDrawing.js` | 行星图 Canvas 绘制全集（createPlanetDrawing getState 工厂） |
...
| 80 | `src/renderer/src/composables/useMarkerEditor.js` | 标记属性编辑器 |
...
| 278 | `src/renderer/src/utils/canvasIcon.js` | Canvas 端矢量图标（Path2D 复刻 Icon.vue 几何 + 旧 emoji 数据回退） |
