...
| 179 | `scripts/migrate-mapdata-keys.js` | mapdata.json 旧（无世界前缀）key 清理：dry-run 报告 + --apply 备份/存档/删除 |
...
| 710 | `src/main/index.js` | 主进程入口：28 个 IPC handle + 窗口/单实例锁/关闭拦截 |
...
| 414 | `src/renderer/src/composables/usePlanetHeightBrush.js` | （待补） |
...
| 340 | `src/renderer/src/composables/useTerrainCanvasBrush.js` | （待补） |
...
| 178 | `src/renderer/src/store/layers.js` | 图层可见性栈 |
